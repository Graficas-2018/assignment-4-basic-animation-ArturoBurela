...
function playAnimations() {
  ...
  for (var i = 0; i <= 50; i++){
    verts.push({
      x: 50*Math.cos(i*2*Math.PI/50),
      y: -4,
      z: 50*Math.sin(i*2*Math.PI/50)}
    );
    keyCont += (1/50);
    k.push(keyCont);
    //console.log(verts[i]);
  }
  carAnimator = new KF.KeyFrameAnimator;
  carAnimator.init({
      interps:
          [
              {
      ...
      loop: true,
      duration:10000,
      easing:TWEEN.Easing.CatmullRom,
  });
  carAnimator.start();
}
...
function createScene(canvas) {
    ...
    mtlLoader.load( url, function( materials ) {
      ...
      objLoader.load( 'SCI_FRS_13_HD.obj', function ( object ) {
